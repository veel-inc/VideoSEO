...
class TrendingSearchModel(BaseModel):
    ...
    unique_query_count: int = Field(
        ..., description="Number of unique query variations"
    )
...
class TrendingSearchRequestModel(BaseModel):
    ...
    limit: Optional[int] = Field(
        20, ge=1, le=100, description="Maximum trends to return"
    )
    min_score: Optional[float] = Field(
        0.0, ge=0.0, description="Minimum trend score threshold"
    )
...
class ExtendedTrendingSearchResponseModel(BaseModel):
    ...
    trends: List[TrendingSearchModel] = Field(
        default_factory=list, description="List of trending searches"
    )
...
class TrendingSearchResponseModel(BaseModel):
    ...
    trending_searches: List[str] = Field(
        ..., description="List of decorated trending search queries"
    )
    error: Optional[str] = Field(None, description="Error message if any")
